...
+ дрон должен выполнить задание, если нет явных препятствий (вред человеку, природе, развернутая система ПВО или РЭБ, техническая неисправность)
+ получение задания только от доверенного управляющего центра
+ в случае многократных попыток неавторизованного доступа вернуться на базу
+ дрон должен быть отказоустйчивым, то есть отказ одного из модулей либо передача недостоверных данных от одного из модулей, не приводит к отказу в обслуживании
Цель безопасности управляющего центра
+ управляющий центр получает данные и управляет только доверенными дронами
+ проверяет отсутствие конфликтов в полётных заданиях
Цель безопасности Client API
+ взаимодействие только с доверенным центром управления
...
|----|----|----|
| *Auth* | Сервис, используемый для установления защищенного соеденинения | Верифицирует сообщения по PSK. Использует AES в режиме EBC с PKCS и HMAC для установления достоверности сообщений, получаемых с УЦ |
...
Доверенными подсистемами можно считать *Auth* и *Fly_control*, компрометация *Fly_control* ведет к невозможности управления летальным средством без резервирования, компрометация *Auth* ведёт к невозможности аутентифицировать источник команд.

Остальные сервисы мы считаем недоверенными, так как в них используются готовые или чистично модифицированные проприетарные решения сторонних производителей.

![Диаграмма объектов](images/subsystem_arch_trusted.png)
