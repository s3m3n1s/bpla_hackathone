...
ЦБД-5 | дрон должен быть отказоустйчивым, то есть отказ одного из модулей либо передача недостоверных данных от одного из модулей, не приводит к отказу в обслуживании
...
+ процедура включения БПЛА управляющим центром доверена
+ любые действия, совершаемые аутентифицированным оператором, а так же принимаемые им решения, доверены
+ невозвращение дрона на базу в случае аварийной посадки считается допустим событием 
...
NS-2 | Компрометация аунтификационных или сессионных данных оператора. Или SSL-проксирование оператора. Получение логина и пароля, либо JWT токена позволит злоумышленнику перехватить управление.
...
NS-4 | Отказ модуля питания. В результате аппаратной/программной закладки в *HW_control*-модуле может привести к откючению батареи при выполнение полётного задания, что приведёт к падению БПЛА.
NS-5 | Отказ модуля питания. Отключения батареи при выполнение полётного задания ведёт к падению БПЛА.
NS-6 | Отказ модуля либо подмена данных навигации.  В результате аппаратной/программной закладке более, чем в одном из модулей навигации, может привести к угону БПЛА.
...
|----|----|----|
| *Auth* | Сервис, используемый для установления защищенного соеденинения | Верифицирует и расшифровывает сообщения по PSK. Использует AES в режиме EBC с PKCS и HMAC для установления достоверности сообщений, получаемых с УЦ |
...
Доверенными подсистемами можно считать *Auth* и *Fly_control*, компрометация *Fly_control* ведет к невозможности управления летальным средством без резервирования (использование другого сервиса в качстве второго *Fly_control*), компрометация *Auth* ведёт к невозможности аутентифицировать источник команд.


Остальные сервисы мы считаем недоверенными, так как в них используются готовые или чистично модифицированные проприетарные решения сторонних производителей. Можно рассмотреть влияние компрометации отдельного взятого сервиса на ЦБ дрона.


Таблица возможных нарушений ЦБ дрона, в зависимости от взломанного (скомпроментированного) сервиса
...
Так же можно рассмотреть возможные нарушения ЦБ дрона, в зависимости от взломанных (скомпроментированных) пар сервисов, которые носят близкие функциональные предназначения
