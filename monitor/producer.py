...
def proceed_to_deliver(id, details):
    ...
    print(f"[MONITOR] [debug] queueing for delivery event id: {id}, payload: {details}")
...
def producer_job(_, config, requests_queue: multiprocessing.Queue):
    ...
    def delivery_callback(err, msg):
        if err:
            print('[MONITOR] [error] Message failed delivery: {}'.format(err))
        # else:
    ...
    while True:
        ...
        try:
            producer.produce(topic, json.dumps(event_details), str(event_details['id']),
            ...
            print('[MONITOR] [error] Message failed encoding: {}'.format(err))
